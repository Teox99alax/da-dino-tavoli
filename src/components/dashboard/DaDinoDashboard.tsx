...
import { AlertTriangle, BarChart3, CheckCircle2, Clock3, CloudRain, LogOut, MapPinned, Search, Trash2, Users, Utensils, Zap, FileText, History, Lightbulb } from "lucide-react";
...
type ServiceSuggestion = { type:string; message:string; priority:"alta"|"media"|"bassa" };
type TimelineEvent = { time:string; type:string; text:string; priority:"alta"|"media"|"bassa" };
...
const suggestions=useMemo(()=>options.map(scoreOption).filter(Boolean).sort((a:any,b:any)=>b.score-a.score).slice(0,12) as ScoredOption[],[form,settings,activeReservations,editingReservationId]);
...
const capacityByArea={sala:27,saletta:18,dehor:settings.awning==="aperte"?34:30,marciapiede:settings.weather==="pioggia"?0:15,esterno:settings.weather==="pioggia"?0:36};const totalCapacity=capacityByArea.sala+capacityByArea.saletta+capacityByArea.dehor+capacityByArea.marciapiede+capacityByArea.esterno;const activeAdults=activeReservations.reduce((s,r)=>s+r.adults,0);const freeSeats=Math.max(totalCapacity-activeAdults,0);const firstTurn=enriched.filter(r=>r.turn==="primo turno"&&isActiveStatus(r.status));const secondTurn=enriched.filter(r=>r.turn==="secondo turno"&&isActiveStatus(r.status));const firstTurnFree=Math.max(totalCapacity-firstTurn.reduce((a,r)=>a+r.adults,0),0);const secondTurnFree=Math.max(totalCapacity-secondTurn.reduce((a,r)=>a+r.adults,0),0);const unassignedReservations=enriched.filter(r=>isActiveStatus(r.status)&&(r.table==="Da assegnare"||r.optionId==="da-assegnare"||!r.moduleIds||r.moduleIds.length===0));const filteredReservations=enriched.filter(r=>{const q=search.trim().toLowerCase();return !q||r.name.toLowerCase().includes(q)||r.phone.toLowerCase().includes(q)||r.table.toLowerCase().includes(q)});

const serviceSuggestions=useMemo<ServiceSuggestion[]>(()=>{
  if(!isMatteo)return [];
  const list:ServiceSuggestion[]=[];
  const sorted=[...enriched].filter(r=>r.status!=="no_show"&&r.status!=="liberato").sort((a,b)=>a.time.localeCompare(b.time));
  const unassigned=sorted.filter(r=>r.table==="Da assegnare"||r.optionId==="da-assegnare"||!r.moduleIds?.length);
  if(unassigned.length>0)list.push({type:"assegnare",priority:"alta",message:`${unassigned.length} prenotazioni sono ancora da assegnare.`});
  sorted.forEach(r=>{
    const total=(r.adults||0)+(r.highchairs||0);
    const history=customerStats.get(normalizeKey(r.name,r.phone));
    if(total>=6)list.push({type:"gruppo",priority:"alta",message:`Preparare ${r.table} per ${r.name}: ${total} persone alle ${r.time}.`});
    if(r.highchairs>0)list.push({type:"seggiolone",priority:"media",message:`Verificare ${r.highchairs} seggiolone/i per ${r.name} al tavolo ${r.table}.`});
    if(r.category==="molto_importante")list.push({type:"vip",priority:"alta",message:`Cliente VIP alle ${r.time}: ${r.name}, tavolo ${r.table}.`});
    if(history&&history.visits>=3)list.push({type:"storico",priority:"bassa",message:`${r.name} è cliente abituale: ${history.visits} visite registrate, ultima il ${history.lastVisit}.`});
    if(r.status==="seduto"&&r.turn==="primo turno"&&toMin(r.resetEnd)>toMin("21:00"))list.push({type:"turno",priority:"alta",message:`Controllare ${r.table}: primo turno pronto stimato ${r.resetEnd}, rischio pressione sul secondo turno.`});
    if(r.notes)list.push({type:"note",priority:"media",message:`Nota per ${r.name}: ${r.notes}`});
  });
  for(let i=0;i<sorted.length-1;i++){
    const a=sorted[i];const b=sorted[i+1];
    const diff=toMin(b.time)-toMin(a.time);
    if(diff>=0&&diff<=15)list.push({type:"picco",priority:"media",message:`Picco arrivi tra ${a.time} e ${b.time}: ${a.name} e ${b.name}.`});
  }
  const byTime:Record<string,number>={};
  sorted.forEach(r=>{byTime[r.time]=(byTime[r.time]||0)+(r.adults||0)+(r.highchairs||0)});
  Object.entries(byTime).forEach(([time,people])=>{if(people>=14)list.push({type:"coperti",priority:"alta",message:`Molti coperti alle ${time}: ${people} persone previste.`})});
  return list.slice(0,12);
},[isMatteo,enriched,customerStats]);

const timelineEvents=useMemo<TimelineEvent[]>(()=>{
  if(!isMatteo)return [];
  const events:TimelineEvent[]=[];
  enriched.filter(r=>r.status!=="no_show"&&r.status!=="liberato").forEach(r=>{
    const total=(r.adults||0)+(r.highchairs||0);
    events.push({time:r.time,type:r.status==="confermata"?"arrivo":"stato",priority:total>=6?"alta":"media",text:`${r.name} · ${r.table} · ${total} persone · ${r.status}`});
    events.push({time:r.estimatedEnd,type:"fine stimata",priority:r.turn==="primo turno"&&toMin(r.resetEnd)>toMin("21:00")?"alta":"bassa",text:`${r.table} · ${r.name} · fine stimata ${r.estimatedEnd}`});
    events.push({time:r.resetEnd,type:"pronto",priority:r.turn==="primo turno"?"media":"bassa",text:`${r.table} ridabile dopo reset · ${r.name}`});
  });
...
function ReservationTable({data,updateStatus,removeReservation,startEditReservation}:{data:any[];updateStatus:(id:number,status:Status)=>void;removeReservation:(id:number)=>void;startEditReservation:(r:Reservation)=>void}){return <div className="overflow-x-auto"><table className="w-full text-sm"><tbody>{data.length===0&&<tr><td className="py-3 text-gray-500">Nessuna prenotazione</td></tr>}{data.map(r=><tr key={r.id} className="border-b align-top"><td className="py-2 font-medium">{r.time}</td><td>{r.name}<div className="text-xs text-gray-500">{r.phone}</div></td><td>{r.adults}{r.highchairs?` + ${r.highchairs} seg.`:""}</td><td>{r.table}<div className="text-xs text-gray-500">{r.estimatedEnd}/{r.resetEnd}</div>{r.suggestedWaitMinutes? <div className="text-xs text-blue-700">attesa {r.suggestedWaitMinutes} min</div>:null}</td><td><span className={`text-xs px-2 py-1 rounded-full ${statusClass(r.status)}`}>{r.status}</span></td><td className="space-x-1 whitespace-nowrap"><Button size="sm" variant="outline" onClick={()=>startEditReservation(r)}>Modifica</Button><Button size="sm" variant="outline" onClick={()=>updateStatus(r.id,"arrivato")}>Arrivato</Button><Button size="sm" variant="outline" onClick={()=>updateStatus(r.id,"seduto")}>Seduto</Button><Button size="sm" variant="outline" onClick={()=>updateStatus(r.id,"pagato")}>Pagato</Button><Button size="sm" variant="outline" onClick={()=>updateStatus(r.id,"liberato")}>Liberato</Button><Button size="sm" variant="outline" onClick={()=>updateStatus(r.id,"no_show")}>No-show</Button><Button size="sm" variant="outline" onClick={()=>removeReservation(r.id)}><Trash2 className="w-4 h-4"/></Button></td></tr>)}</tbody></table></div>}
