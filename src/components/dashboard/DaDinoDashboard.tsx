...
function displayMapTableNumber(m:TableModule, occ:any){
  if (occ?.moduleIds?.length) {
    const first=[...(occ.moduleIds||[])].sort((a,b)=>moduleNumber(a)-moduleNumber(b))[0];
    const n=moduleNumber(first);
    if(Number.isFinite(n)&&n!==999)return String(n);
  }

  if (occ?.table) {
    const n=tableNumber(String(occ.table));
    if(Number.isFinite(n)&&n!==999)return String(n);
  }
